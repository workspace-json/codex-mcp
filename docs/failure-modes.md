...
| GPT-5.6 reviewer unavailable, not installed, or not invoked † | No effect on enforcement. `hooks/pre-edit-check.mjs` contains no reference to the reviewer at all, so it denies/warns/annotates identically with or without it. |
| GPT-5.6 reviewer returns `PASS` † | Advisory only — does not change, weaken, or bypass the hook's decision on any file. Its own instructions state the verdict "must never alter the hook decision"; no code path exists that would let it. |
